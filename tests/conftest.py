...
def app():
    ...
    with app.app_context():
        ...
        client = Client(name='Test', surname='User',
                        credit_card='1234567890123456', car_number='A123BC')
        ...
        # Лог заезда и выезда
        cp = ClientParking(client_id=client.id, parking_id=parking.id, time_in=datetime.now() - timedelta(hours=1), time_out=datetime.now())
        _db.session.add(cp)
        _db.session.commit()
